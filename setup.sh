...
sudo docker build -t cnid_infra_image .	
...
sudo docker service rm cnid_infra_service 2>&1 > /dev/null

# Create service for cnid_infra
sudo docker service create --name cnid_infra_service -p 80:80 --replicas 3 cnid_infra_image
